...
print(f"\n🔍 DEBUG - COLUNAS APÓS SESSÃO 3:")
for arquivo, abas_dict in arquivos_filtrados_limpos.items():
    if 'Lead score LF 24.xlsx' in arquivo or 'LF 24' in arquivo:
        for aba_nome, df_limpo in abas_dict.items():
            if 'LF Pesquisa' in aba_nome or 'Pesquisa' in aba_nome:
                print(f"\nArquivo: {arquivo}")
                print(f"Aba: {aba_nome}")
                print(f"Total de colunas: {len(df_limpo.columns)}")
                print("Colunas restantes após limpeza:")
                for i, col in enumerate(sorted(df_limpo.columns), 1):
                    print(f"  {i:2d}. {col}")
                break
        break
...
print("UNIFICAÇÃO DE UTM MEDIUM BASEADA EM ACTIONS + TRATAMENTO DE PRODUÇÃO")
print("=" * 72)
...
def relatorio_unificacao_producao(df_original, df_unificado):
...
# Disponibilizar dataset final
pesquisa_medium_producao_unificado = df_medium_producao
...
def criar_versoes_dataset():
    ...
    df = pesquisa_medium_producao_unificado.copy()
...
"""## 22- Investigação de que modelo funciona melhor, usando:
...
"""##22.5 - Investigação se as UTMs realmente estão no dataset final"""
...
"""## 22.6- Investigacão se modelos treinados nos dados completos são realmente superiores aos temporais."""
