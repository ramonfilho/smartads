...
def create_professional_motivation_score(df, text_cols, fit=True, params=None):
    ...
    df_result['professional_motivation_score'] = normalized_scores
    df_result['career_keyword_count'] = keyword_counts
    
    return df_result, params

def analyze_aspiration_sentiment(df, text_cols, fit=True, params=None):
    ...
    # Frases de aspiração para detectar - Lista EXATA do script 06
    aspiration_phrases = [
    ...
    for col in text_cols:
        ...
        # Extrair componentes
        df_result[f"{col_clean}_sentiment_pos"] = [score["pos"] for score in sentiment_scores]
        df_result[f"{col_clean}_sentiment_neg"] = [score["neg"] for score in sentiment_scores]
        df_result[f"{col_clean}_sentiment_compound"] = [score["compound"] for score in sentiment_scores]
        ...
        df_result[f"{col_clean}_aspiration_count"] = aspiration_counts
        
        # Score de aspiração: combinação de sentimento positivo e frases de aspiração
        df_result[f"{col_clean}_aspiration_score"] = np.array(aspiration_counts) * \
                                                 df_result[f"{col_clean}_sentiment_pos"]
    
    return df_result, params

def detect_commitment_expressions(df, text_cols, fit=True, params=None):
    ...
    # Frases de compromisso em espanhol com pesos - Lista EXATA do script 06
    commitment_phrases = {
    ...
    for col in text_cols:
        ...
        df_result[f"{col_clean}_commitment_score"] = commitment_scores
        df_result[f"{col_clean}_has_commitment"] = has_commitment
        df_result[f"{col_clean}_commitment_count"] = commitment_counts
    
    return df_result, params

def create_career_term_detector(df, text_cols, fit=True, params=None):
    ...
    # Termos relacionados à carreira com pesos - Lista EXATA do script 06
    career_terms = {
    ...
    for col in text_cols:
        ...
        # Adicionar features ao DataFrame
        df_result[f"{col_clean}_career_term_score"] = career_scores
        df_result[f"{col_clean}_has_career_terms"] = has_career_terms
        df_result[f"{col_clean}_career_term_count"] = career_term_counts
...
def enhance_tfidf_for_career_terms(df, text_cols, fit=True, params=None):
    ...
    # Termos relacionados à carreira para dar boost - Lista EXATA do script 06
    career_terms = [
...
def enhance_professional_features(df, text_cols, fit=True, params=None):
    ...
    df_result, params = create_professional_motivation_score(df, text_cols, fit, params)
    
    # 2. Aprimorar TF-IDF para termos de carreira
    print("2. Aprimorando TF-IDF para termos de carreira...")
    df_tfidf, params = enhance_tfidf_for_career_terms(df, text_cols, fit, params)
    
    # 3. Análise de sentimento de aspiração
    print("3. Analisando sentimento de aspiração...")
    df_aspiration, params = analyze_aspiration_sentiment(df, text_cols, fit, params)
    
    # 4. Detecção de expressões de compromisso
    print("4. Detectando expressões de compromisso...")
    df_commitment, params = detect_commitment_expressions(df, text_cols, fit, params)
    
    # 5. Detector de termos de carreira
    print("5. Criando detector de termos de carreira...")
    df_career, params = create_career_term_detector(df, text_cols, fit, params)
